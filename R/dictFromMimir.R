...
#' @param table data.table or data.frame (must incluse "HELM", "TS_BASE_SEQ", 
...
mimir2XnaDict <- function(table, base.col, sugar.col, backbone.col) {
  ...
  dt <-
    data.table::as.data.table(eval(parse(
      text = paste(
        "table[, c('HELM','",
        base.col,
        "','",
  ...
  setnames(
    dt,
    old = c(base.col, sugar.col, backbone.col),
    new = c("base", "sugar", "backbone")
  )
  dt <-
    data.table::melt(
      data = dt,
      id.vars = "HELM",
      measure.vars = c("base", "sugar", "backbone"),
      variable.name = "type",
      value.name = "symbol"
    )
