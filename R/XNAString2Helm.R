...
XNAStringToHelm <-
  function(xnastring_obj,
    ...
    indx_base <- stats::setNames(as.character(base_dict$HELM), base_dict$symbol)
    indx_sugar <- stats::setNames(as.character(sugar_dict$HELM), sugar_dict$symbol)
    indx_backbone <- stats::setNames(as.character(backbone_dict$HELM), backbone_dict$symbol)
    
    base <- as.character(xnastring_obj@base)
    sugar <- as.character(xnastring_obj@sugar)
    backbone <- as.character(xnastring_obj@backbone)
    conjugate5 <- xnastring_obj@conjugate5
    conjugate3 <- xnastring_obj@conjugate3
    
    helms <- sapply(seq(1, length(base)), function(strand_nr){
      base_trans <- vapply(strsplit(as.character(strsplit(base[strand_nr], '')[[1]]), '>'), function(x) 
        paste(indx_base[x], collapse=">"), character(1L))
      
      sugar_trans <- vapply(strsplit(as.character(strsplit(sugar[strand_nr], '')[[1]]), '>'), function(x) 
        paste(indx_sugar[x], collapse=">"), character(1L))
      
      backbone_trans <- vapply(strsplit(as.character(strsplit(backbone[strand_nr], '')[[1]]), '>'), function(x) 
        paste(indx_backbone[x], collapse=">"), character(1L))
      # add '' manually as the last symbol
      backbone_trans[length(backbone_trans) +1] <- ''
      
      parsed_xna_obj <- paste(sugar_trans, base_trans, backbone_trans, sep='')
    ...
    # if base is a 2-elements vector, add pairing information with siRNA_HELM function
    if(length(base) == 2){
      helms <- paste(paste0(helms, collapse ='|'), "$", siRNA_HELM(xnastring_obj), "$$$V2.0", sep = '')
    } else {
...
siRNA_HELM <- function(xnastring_obj){
  ...
  length(base) == 2 ||
    stop("Pairing infrmormation can be only returned for double stranded molecules")
  
  base_sense <- changeBase(xnastring_obj@compl_dictionary, base[1])
  base_antisense <- changeBase(xnastring_obj@compl_dictionary, base[2])
  
  mat <- Biostrings::nucleotideSubstitutionMatrix(match = 1, mismatch = 0, baseOnly = TRUE)
  
  pa <- Biostrings::pairwiseAlignment(base_sense, 
  ...
  pair_str <- paste(paste0('RNA1,RNA2,',p1*3-1,':pair-',p2*3-1,':pair'),collapse='|')
