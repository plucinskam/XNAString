...
XNAStringToHelm <-
  function(xnastring_obj,
    ...
    helms <- vapply(seq(1, length(base)), function(strand_nr) {
      base_trans <-
        vapply(strsplit(as.character(strsplit(base[strand_nr], "")[[1]]), ">"),
               function(x) {
                 paste(indx_base[x], collapse = ">")
               }, character(1L))
      
      sugar_trans <-
        vapply(strsplit(as.character(strsplit(sugar[strand_nr], "")[[1]]), ">"),
               function(x) {
                 paste(indx_sugar[x], collapse = ">")
               }, character(1L))
      
      backbone_trans <-
        vapply(strsplit(as.character(strsplit(backbone[strand_nr], "")[[1]]),
                        ">"),
               function(x) {
...
siRNA_HELM <- function(xnastring_obj) {
  ...
  length(base) == 2 ||
    stop("Pairing infrmormation can be only returned for double
         stranded molecules")
  
  base_sense <- changeBase(xnastring_obj@compl_dictionary, base[1])
  base_antisense <-
    changeBase(xnastring_obj@compl_dictionary, base[2])
